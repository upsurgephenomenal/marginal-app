// Backs the /api/claude route by proxying to Cloudflare Workers AI
// (env.AI), Cloudflare's own hosted open-source models. Runs on
// Cloudflare's free daily Neuron allocation — no external API key, no
// billing account, no credit card required anywhere in this stack.
//
...
export async function handleClaude(request, env) {
  if (!env.AI) {
    return json({ error: "Workers AI binding (AI) is not configured on this Worker. Add it under the project's Bindings tab." }, 500);
  }
