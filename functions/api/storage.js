...
// A minimal key-value store backed by a Cloudflare KV namespace bound
// as MARGINAL_KV. Personal data (shared=false) is namespaced under the
...
export async function onRequest(context) {
  ...
  if (!env.MARGINAL_KV) {
    return json({ error: "MARGINAL_KV is not bound on this Pages project. Add a KV namespace binding under Settings > Functions." }, 500);
  }

  // Cloudflare Access injects this header once an Access policy is
  // attached to the project. Locally (no Access) it falls back to a
  // single shared "local" user so you can still test.
